...
namespace Steps\WsLoginHook\Services;

use Steps\WsLoginHook\Models\OtpRequest;
...
class WhatsappAuthHandler
{
    ...
    public function generateOtp($serialNumber = null, $model)
    {
        ...
        if ($serialNumber) {
            $requestData = OtpRequest::where('serial_number', $serialNumber)
                ->where('model_type', config("ws-login-hook.supported_models.$model"))->first();
        ...
        if (! $requestData) {
            $requestData = OtpRequest::create([
                ...
                'model_type' => config("ws-login-hook.supported_models.$model"),
            ]);
        }
    ...
    public function handleWsWebhook(Request $request): ?array
    {
        ...
        if (! $message || ($message['type'] ?? null) !== 'text') {
            \Log::channel('info')->info('WS Webhook message not found');
            return null;
        }
    ...
    {
        $reciverNumber = preg_replace('/\D/', '', config('ws-login-hook.whatsapp_reciver_number'));
